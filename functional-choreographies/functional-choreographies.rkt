...
(define-syntax (branch? stx)
  (lambda (stx)
    (raise-syntax-error
     #f
     "Encountered unexpected `branch?` form!"
     stx)))
                      ...
                      (for/list ([pair (hash->list label-union)])
                        (let ([label (car pair)]
                          ...
                          (if (pair? expr)
                              ...
                              (let* ([expr1 (car expr)]
                                     [expr2 (cdr expr)])
                                #`[#,label (merge #,expr1 #,expr2)])
                              ;; The case where a label is in only one branch
                              (begin
                                #`[#,label #,expr])))))]))

(define-syntax (found-branch? stx)
  (syntax-parse stx
    [(_ SEND-PROC [LABEL GEXPR] ...)
     #`(let ([recv-label (recv #,(fmt-net-proc #'SEND-PROC) 'void)])
         (cond
           #,@(for/list ([label (syntax->list #'(LABEL ...))]
                         [gexpr (syntax->list #'(GEXPR ...))])
                #`[(equal? #,label recv-label)
                   #,gexpr])))]))

(define-for-syntax (match-stx left-stx right-stx)
  (syntax-parse #`(#,left-stx #,right-stx) #:literals (branch?)
    [((branch? _ ...) (branch? _ ...))
     (merge-branches left-stx right-stx)]
    [((L1 L2 LR ...) (R1 R2 RR ...))
